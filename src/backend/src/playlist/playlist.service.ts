...
export class PlaylistService {
  ...
  async create(playlist: PlaylistEntity, username: string): Promise<void> {
    ...
    if (isTrack) {
      await this.createSingleTrack({ ...playlist, username });
    } else {
      await this.createPlaylist({ ...playlist, username });
    }
  }
